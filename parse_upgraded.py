...
while True:
    ...
    file_path = "bestbuy_{search_item}_items.txt".format(search_item=search_item)
    if len(products) < 1:
        break
        ...
        for items in products:
            ...
            if search_item == "laptop":
                ...
                for element in string_to_search:
                    if re.search('(\d+GB)|(\d+TB)', element):
                        if "GB Memory" not in element:
                            hard_drive = re.sub(r'\([\w\s]+\)', '', element)
                            # print('"'+ hard_drive +'"')
                    page_line = '"{brand}"\t"{screen_size}"\t"{hard_drive}"\t"{color}"\n'.format(brand=brand, color=color, hard_drive=hard_drive,screen_size=screen_size)
