    ...
    with open(file_path, "a", encoding='utf8') as textfile:
        ...
        for items in products:
            ...
            if search_item == "laptop" or search_item == "desktop pc":
                ...
                new_model2 = re.sub('\s*Laptop\s*','',new_model)
                ...
                processor = quick_search('AMD\sRyzen\s\w+|AMD\s\w+-\w+|Intel\s\w+\s?\w*', product_name)
                video_card = quick_search('AMD Radeon[\s\w]+[^\s-]|NVIDIA[\s\w]+-?[^\s]', product_name)
                for element in string_to_search:
                    if hd_pattern.search(element):
                        if "GB Memory" not in element:
                            ...
                            print(element)
                print(new_model2)
                page_line = '"{brand}"\t"{product_model}"\t"{screen}"\t"{model}"\t"{processor_name}"\t"{ram}"\t"{video}"\t"{hard_drive}"\t"{color}""' \
                            '\n'.format(brand=brand, model=new_model2.replace('Refurbished', '').strip(), video=video_card.replace(' -', '').strip(), color=color, processor_name=processor.strip(), ram=ram,
                ...
                page_line = '"{brand}"\t"{product_model}"\t"{screen_size}"\t"{tv_class}"\t"{resolution}"' \
                            '\t"{tv_series}"\t"{description}"'\
                            '\n'.format(product_model=product_model, resolution=resolution, brand=brand.replace('TV','').strip(), screen_size=screen.replace('"', ''), tv_class=tv_class, tv_series=tv_series, description=description)
                textfile.write(page_line)

            elif search_item == "smartphone":
                parsed_product_name= product_name.split(' - ')
                brand = parsed_product_name[0]
                color = re.sub('\([\w\s]+\)','', parsed_product_name[-1])
                model_found = quick_search('.+(?=\swith)|.+GB|.+(?=Cell)', parsed_product_name[1])
                model = re.sub(r'\s\d+GB$', '', model_found)
                memory = quick_search('\s\d+GB', parsed_product_name[1])

                page_line = '"{brand}"\t"{model}"\t"{memory}"\t"{color}"\t"{model_number}"' \
                            '\n'.format(model=model.replace('Pre-Owned', '').replace('Refurbished', '').replace('(Excellent)', '').replace("(Unlocked)", '')
                                        .replace("Geek Squad Certified", '').replace('Certified', '').replace('Excellent','').replace('4G LTE','').replace('4G','').replace('LTE','').strip(),
                                        brand=brand.strip(),
                                        memory=memory.strip(),
                                        color=color.replace('(AT&T)', '').strip(),
